...
def get_quote_yahoo(symbols):
    ...
    request = str.join('', list(codes.values()))  # code request string
    header = list(codes.keys())

    data = dict(list(zip(list(codes.keys()), [[] for i in range(len(codes))])))
...
def get_components_yahoo(idx_sym):
    ...
    while (True in mask):
        ...
        lines = (urllib.request.urlopen(urlStr).read().decode('utf-8').strip().
                 strip('"').split('"\r\n"'))
...
def get_data_fred(name=None, start=dt.datetime(2010, 1, 1),
    ...
    data = read_csv(urllib.request.urlopen(url), index_col=0, parse_dates=True,
...
def get_data_famafrench(name, start=None, end=None):
    ...
    url = urllib.request.urlopen(zipFileURL + name + ".zip")
...
class Options(object):
    ...
    def get_near_stock_price(self, above_below=2, call=True, put=False,
        ...
        if call:
            ...
            get_range = list(range(start_index - above_below,
